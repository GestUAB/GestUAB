...
//            {
                ...
                for (var i = 0; i < allProperties.Count(); i++)
                {
                    ...
                    if (keyProperties.Contains(property) && property.PropertyType != typeof(Guid)) continue;
...
//            }
        ...
        /// <param name="entityToInsert">Entity to insert</param>
        /// <returns>Identity of inserted entity</returns>
        public static long Insert(this IDbConnection connection, dynamic entityToInsert, string tableName, IDbTransaction transaction = null, int? commandTimeout = null) 
        {
            //            using (var tx = connection.BeginTransaction())
            //            {
            ...
            var allProperties = entityToInsert as IDictionary<string, object>;
            //var keyProperties = KeyPropertiesCache(type);

            int i = 0;
            foreach (var property in allProperties)
            {
                if (property.Key == "Id" && !(property.Value is Guid)) continue;
            ...
            sb.Append(") ");
            var pars = new DynamicParameters(entityToInsert);
            connection.Execute(sb.ToString(), pars, transaction: transaction, commandTimeout: commandTimeout);
            //NOTE: would prefer to use IDENT_CURRENT('tablename') or IDENT_SCOPE but these are not available on SQLCE
            //var r = connection.Query("select @@IDENTITY id");
            //tx.Commit();
            //return (int)r.First().id;
